...
gss_snapchat <- gss %>%
 filter(snapchat!="NA") %>% #we need to remove the NA values
  group_by(snapchat) %>% 
  summarise(count = n()) %>% 
...
gss_twitter <- gss %>%
 filter(twitter!="NA") %>% #we need to remove the NA values
  group_by(twitter) %>% 
  summarise(count = n()) %>% 
...
#We add an error bar to better visualise the confidence intervals for both male and female
